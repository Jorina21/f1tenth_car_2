...
class QualifierNode(Node):
    def __init__(self):
        ...
        self.declare_parameters(
            ...
            parameters=[
                ...
                ("min_speed", 0.35),
                ("recovery_speed", 0.35),
                ("speed_scale", 0.5), # cuts speed in half
        ...
        self.speed_scale = clamp(self.speed_scale, 0.0, 1.2)
    ...
    def parameter_callback(self, params):
        for param in params:
            if param.name == "speed_scale":
                try:
                    value = float(param.value)
                except (TypeError, ValueError):
                    return SetParametersResult(
                        successful=False,
                        reason="speed_scale must be a number",
                    )

                if value < 0.0 or value > 1.2:
                    return SetParametersResult(
                        successful=False,
                        reason="speed_scale must be between 0.0 and 1.2",
                    )

                self.speed_scale = value
                self.get_logger().info(
                    f"Updated speed_scale: {self.speed_scale:.2f}"
                )

        return SetParametersResult(successful=True)

    def apply_speed_scale(self, speed):
        # Only scale forward autonomy speed.
        # Reverse recovery stays unchanged so the car can still back out normally.
        if speed <= 0.0:
            return speed
        ...
        return clamp(scaled_speed, 0.0, max_speed)
    ...
    def control_loop(self):
        ...
        speed = self.apply_speed_scale(speed)
