...
class F1TenthRaceDayNode(Node):
    def __init__(self):
        ...
        self.pass_target_offset = 0.0
        self.pass_clear_start_time = 0.0
    ...
    def declare_all_params(self):
        params = [
            ...
            # Frenet planner
            ("frenet_max_offset", 0.50),
            ("frenet_num_offsets", 5),
            ("frenet_horizon", 2.5),
            ("frenet_step", 0.20),
            ("path_corridor_width", 0.35),
            ("offset_change_rate", 0.90),

            # Obstacle / opponent
            ("opponent_detect_distance", 2.5),
            ("safe_follow_distance", 0.5),
            ...
            # Passing behavior
            ("pass_commit_time", 1.2),
            ("pass_clear_distance", 1.0),
            ("pass_hold_extra_time", 0.8),
            ("force_full_pass_offset", True),
            ("switching_penalty", 5.0),
            ("pass_offset_threshold", 0.08),
            ("return_offset_tolerance", 0.05),

            # Cost weights
            ("raceline_cost_weight", 0.15),
            ("collision_cost_weight", 12.0),
            ("smoothness_cost_weight", 0.5),
            ("progress_reward_weight", 1.0),
            ("pass_reward_weight", 4.0),
    ...
    def load_params(self):
        ...
        self.pass_clear_distance = float(gp("pass_clear_distance").value)
        self.pass_hold_extra_time = float(gp("pass_hold_extra_time").value)
        self.force_full_pass_offset = bool(gp("force_full_pass_offset").value)
    ...
    def control_loop(self):
        ...
        if self.debug_lidar and now_sec - self.last_lidar_log_time > 0.5:
    ...
    def update_state_machine(self, raceline_blocked, nearest_idx, lidar_info):
        ...
        # Strong pass latch.
        # Once PASS_LEFT/PASS_RIGHT starts, do not return to center until the
        # obstacle is no longer ahead, center is clear, and the hold timers expire.
        if self.state in [RaceState.PASS_LEFT, RaceState.PASS_RIGHT]:
            elapsed = now_sec - self.pass_start_time

            pass_path = self.build_offset_path(nearest_idx, self.pass_target_offset)
            pass_path_blocked = self.is_path_blocked(pass_path)

            # If the committed pass lane becomes unsafe, go back to replan.
            if pass_path_blocked:
                self.state = RaceState.OPPONENT_DETECTED
                self.pass_clear_start_time = 0.0
                return

            center_clear = self.is_center_path_clear(nearest_idx)
            obstacle_ahead = self.obstacle_still_ahead()

            # If object is still ahead, keep passing.
            if obstacle_ahead:
                self.pass_clear_start_time = 0.0
                return

            # If center path is still not clear, keep passing.
            if not center_clear:
                self.pass_clear_start_time = 0.0
                return

            # Minimum commitment time.
            if elapsed < self.pass_commit_time:
                return

            # Start extra clear timer once it first looks safe.
            if self.pass_clear_start_time == 0.0:
                self.pass_clear_start_time = now_sec
                return

            # Hold pass lane a little longer after clear.
            if now_sec - self.pass_clear_start_time < self.pass_hold_extra_time:
                return
    ...
    def select_path(self, nearest_idx, raceline_blocked, dt):
        ...
        # If already passing, stay locked on the committed pass target offset.
        if self.state == RaceState.PASS_LEFT:
            desired_offset = abs(self.pass_target_offset)

            if desired_offset < self.pass_offset_threshold:
                desired_offset = self.frenet_max_offset

        elif self.state == RaceState.PASS_RIGHT:
            desired_offset = -abs(self.pass_target_offset)

            if abs(desired_offset) < self.pass_offset_threshold:
                desired_offset = -self.frenet_max_offset

        elif self.state == RaceState.RACELINE_FOLLOW:
            desired_offset = 0.0
            ...
            if best_candidate is None:
                ...
                # Make pass decisive: full left/right offset after pass decision.
                if self.force_full_pass_offset and abs(desired_offset) > self.pass_offset_threshold:
                ...
                if desired_offset > self.pass_offset_threshold:
                    if self.state != RaceState.PASS_LEFT:
                        ...
                        self.pass_clear_start_time = 0.0
                        self.pass_target_offset = desired_offset
    ...
    def build_offset_path(self, start_idx, offset):
        ...
        while distance < self.frenet_horizon and len(path) < 200:
            ...
            # Positive offset = left side of raceline direction.
    ...
    def obstacle_still_ahead(self):
        """
        True if an obstacle is still in front of the car.
        This prevents early merge-back during PASS_LEFT/PASS_RIGHT.
        """
        lidar_points = self.lidar_points_in_base_frame()
        ...
        for obs in lidar_points:
            ...
            # Object is still in front and close enough to matter.
            if 0.10 < ox < self.pass_clear_distance:
                # Focus near car centerline so side walls do not count.
                if abs(oy) < self.collision_radius + 0.35:
                    return True
    ...
    def is_path_blocked(self, path):
        """
        Corridor-based path blocked detector.
        Better for cones, suitcases, stopped cars, and obstacles between waypoints.
        """
        lidar_points = self.lidar_points_in_base_frame()
    ...
    def lidar_points_in_base_frame(self):
        ...
        for r in self.scan.ranges:
            if np.isfinite(r) and 0.05 < r < max_range:
                ...
                # Keep front and side-front points.
                if x > -0.25:
                    points.append((x, y))
    ...
    def log_state(self, now_sec):
        ...
        self.get_logger().info(
            ...
            f"pass_target={self.pass_target_offset:.2f}"
        )
