...
class WallFollow(Node):
    def __init__(self):
        ...
        self.declare_parameter("beam_a_angle", -50.0)
        self.declare_parameter("beam_b_angle", -90.0)
        self.declare_parameter("K_p", 0.35)
        ...
        self.odom_sub = self.create_subscription(Odometry, odom_topic, self.odom_callback, 7)
    ...
    def follow_wall(self, ranges: List[float], angles: List[float]) -> float:
        ...
        a = ranges[min(max(a_id, 0),len(ranges)-1)]
        b = ranges[min(max(b_id, 0),len(ranges)-1)]
